...
class FCLayer(nn.Module): # 一次维护多个模型的参数
    ''' 集成之后的全连接层 '''
...
class EnsembleDynamicsModel:
    ...
    def __init__(self, state_dim, action_dim, num_network=5):
        ...
        self._best_holdout_loss = float('inf')
    ...
    def train(self,
        ...
        for epoch in itertools.count():
            ...
            with torch.no_grad():
                ...
                holdout_loss = torch.mean(holdout_losses).item()
                if holdout_loss < self._best_holdout_loss:
                    self._best_holdout_loss = holdout_loss
...
class PETS:
    ...
    def __init__(self, env, replay_buffer, n_sequence, elite_ratio,
                 plan_horizon, num_episodes, num_exploration_episodes=5):
        self._env = env
    ...
    def train(self):
        return_list = []
        for i_episode in range(self.num_exploration_episodes):
            explore_return = self.explore()
            print('episode: %d, return: %d' % (i_episode + 1,
                                               explore_return))
...
pets = PETS(env, replay_buffer, n_sequence, elite_ratio, plan_horizon,
            num_episodes, num_exploration_episodes)
